...
/*            LineSearchSolver ls = (_state.l1pen() == 0)
    ...
    private double[] fitCoeffs(int[] randCatLevels, int totRandCatLevels, Frame returnFrame, int[] dinfoWCol, int[] wCol, 
                           int etaOColIndexReturnFrame, int dinfoResponseColID, double[] sumEtaInfo) {
      ...
      cleanupHGLMMemory(null, new Frame[]{augXZ, augZW, qMatrix}, null);
      return sumEtaInfo;
    }
    ...
    private void fitHGLM() {
      ...
      do {
        sumEtaInfo = fitCoeffs(randCatLevels, totRandCatLevels, glmmmeReturnFrame, dinfoWCol, wCol, 
                5, 3, sumEtaInfo);  // loop to fit fixed/random coeffs
        fitDataDispersion(glmmmeReturnFrame, constXYWeight, devHvColIdx);
        estimateRandomCoeffCh(glmmmeReturnFrame, devHvColIdx, cumSumRandLevels, numRandCols, numDataRows, VC2);
      } while (progressHGLMGLMMME(sumEtaInfo[0], sumEtaInfo[1], iter, false));
    ...
    private void estimateRandomCoeffCh(Frame returnFrame, int[] devHvColIdx, int[] cumSumRandLevels, int numRandCols, 
      ...
      for (int colIndex=0; colIndex < numRandCols; colIndex++) {
        Frame constXYWeight = new Frame(makeZeroOrOneFrame(cumSumRandLevels[colIndex], 3, 1,
                new String[]{"response", "X", "weights"}));
        DKV.put(constXYWeight._key, constXYWeight);
        GLMModel glmRand = buildGammaGLM(returnFrame, constXYWeight, devHvColIdx, dev2UseStart, 
                (long) cumSumRandLevels[colIndex], true);  // generate frame to build glm model
        double sigma2u = glmRand.coefficients().get("Intercept");
        // update phi value with glmRand fitted value
        double newPhi = Math.exp(sigma2u);
        ...
        double[] stdErr = glmRand._output.stdErr();
        VC2[colIndex][1] = stdErr[0];
        VC2[colIndex][0] = sigma2u;
      }
    }
    
    public void fitDataDispersion(Frame returnFrame, Frame constXYWeight, int[] devHvColIdx) {
      GLMModel g11 = buildGammaGLM(returnFrame, constXYWeight, devHvColIdx, 0, 
              _dinfo._adaptedFrame.numRows(), false);
      // update tau
      double sigma2e = g11.coefficients().get("Intercept");
      _state.set_tau(Math.exp(sigma2e));
    }
...
//      buildModel();
    ...
    public boolean progressHGLMGLMMME(double sumDiff2, double sumeta2, int iteration, boolean atGLMMME) {
